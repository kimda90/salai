...
import type { ParentRef } from "@salai/script-model";
...
function ParentPicker({
  ...
  value: ParentRef;
  onChange: (parent: ParentRef) => void;
}) {
...
function BeatCard({
  ...
  item: BoardItem;
  reference: Extract<CanonicalReference, { type: "beat" }>;
}) {
  ...
  return (
    ...
    >
      <div className="wall-card-topline">
        ...
        <span className="wall-card-runtime">
          {formatDuration(duration.beatMs[beat.id])}
        </span>
        <button
          ref={handleRef}
        ...
        <button
          ...
          onClick={() =>
            controller.updateWorkspace((workspace) =>
              setBoardItemParking(workspace, item.id, "parked"),
            )
          }
        >
          Park
...
function SceneCard({
  ...
  item: BoardItem;
  reference: Extract<CanonicalReference, { type: "scene" }>;
}) {
  const controller = useSalaiController();
  const { project, selection } = useSalaiState();
  ...
  return (
    ...
    >
      <div className="wall-card-topline">
        ...
        <span className="wall-card-runtime">
          {formatDuration(duration.sceneMs[scene.id])}
        </span>
        <button
          ref={handleRef}
          className="drag-handle"
      ...
      <div className="scene-beat-preview">
        ...
        {scene.beatIds.length > 3 ? (
          <span>+{scene.beatIds.length - 3}</span>
        ) : null}
      </div>
      <div className="wall-card-footer">
        <span>
          {scene.beatIds.length} beat{scene.beatIds.length === 1 ? "" : "s"}
        </span>
        <button
          className="wall-small-button"
          type="button"
...
function IdeaWallCard({ item }: { item: BoardItem }) {
  ...
  return (
    <div
      ref={cardRef}
      className="wall-card idea-wall-card"
      style={{ left: item.x ?? 0, top: item.y ?? 0 }}
    >
      <div className="wall-card-topline">
      ...
      <textarea
        ...
        onChange={(event) =>
          controller.updateWorkspace((workspace) =>
            updateIdeaCardText(workspace, item.id, event.target.value),
          )
        }
      />
      {firstSectionId ? (
      ...
      <div className="wall-card-footer">
        ...
        <button
          ...
          onClick={() =>
            controller.updateWorkspace((workspace) =>
              removeBoardItem(workspace, item.id),
            )
          }
        >
          Delete
...
function ActiveCard({ item }: { item: BoardItem }) {
  ...
  if (item.reference?.type === "beat") {
    return <BeatCard item={item} reference={item.reference} />;
  }
  if (item.reference?.type === "scene") {
    return <SceneCard item={item} reference={item.reference} />;
  }
  return null;
}
...
function StoryBoardCanvas() {
  ...
  const activeItems = workspace.board.itemIds
    ...
    .filter(
      (item): item is BoardItem =>
        Boolean(item) && item.parkingState !== "parked",
    );
  ...
  useEffect(() => {
    ...
    return dropTargetForElements({
      ...
      onDrop: ({ source, location }) => {
        ...
        controller.updateWorkspace((current) =>
          moveBoardItem(current, itemId, x, y),
        );
      },
    });
  }, [controller]);
  ...
  return (
    <div
      ref={boardRef}
      className="story-board-canvas"
      aria-label="Story Wall board"
    >
      ...
      {activeItems.length === 0 ? (
        <div className="empty-board">
          No active cards. Restore one from the parking lot or add an idea.
        </div>
      ) : null}
    </div>
  );
    ...
    <aside className="parking-lot">
      ...
      {parkedItems.length === 0 ? (
        <p className="parking-empty">
          Park uncertain or rejected material here without deleting it.
        </p>
      ) : null}
      <div className="parking-list">
        {parkedItems.map((item) => {
          const label =
            item.ideaCard?.text ||
            (item.reference?.type === "beat"
              ? project.beats[item.reference.id]?.title
              : undefined) ||
            (item.reference?.type === "scene"
              ? project.scenes[item.reference.id]?.title
              : undefined) ||
            item.reference?.id ||
            item.id;
          return (
            <div key={item.id} className="parked-item">
              ...
              <button
                ...
                onClick={() =>
                  controller.updateWorkspace((current) =>
                    setBoardItemParking(current, item.id, "active"),
                  )
                }
              >
                Restore
...
function StructuralOrder() {
  ...
  return (
    <aside className="story-order-panel">
      ...
      <p className="order-explainer">
        This explicit list changes canonical story order. Free card position
        never does.
      </p>
            ...
            {section.childIds.map((childId, index) => {
              ...
              return (
                <div className="order-item" key={childId}>
                  <span>
                    <small>{isScene ? "Scene" : "Beat"}</small>
                    {label}
                  </span>
                  <OrderButtons
                    upDisabled={index === 0}
                    ...
                    onUp={() => {
                      if (isScene) {
                        ...
                        controller.dispatchNarrative({
                          op: "moveBeat",
                          beatId: childId,
                          toParent: { type: "section", id: section.id },
                          toIndex: index - 1,
                        });
                      }
                    }}
    ...
    <div className="surface story-wall-surface">
      <div className="surface-heading">
        <div>
          ...
          <h2>Story Wall</h2>
          <p>
            Move cards freely to think spatially. Use Story order only when you
            intend to change the canonical narrative sequence.
          </p>
        </div>
        <button
          type="button"
      ...
      {project.script.sectionIds.length === 0 ? (
        <div className="wall-notice">
          Create a Section in Outline before promoting IdeaCards.
        </div>
      ) : null}
    </div>
  );
